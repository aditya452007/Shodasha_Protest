...
import { sanitizeToPlainText, validateAndExtractHttpsLinks, hashContent, } from '../../utils/sanitizer.js';
import { calculateTrendingScore } from '../../utils/trendingScore.js';
import { sseService } from '../sse/sse.service.js';
export class PostsService {
    async listPosts(params, voterHash) {
        ...
        const whereClause = and(eq(posts.status, 'active'), categoryId ? eq(posts.categoryId, categoryId) : undefined);
        let orderByClause;
        if (sort === 'latest') {
    ...
    async getPostById(id, voterHash) {
    ...
    async createPost(input, _fingerprint) {
    ...
    async searchPosts(params) {
        ...
        const whereClause = and(eq(posts.status, 'active'), sql `${searchVector} @@ ${searchQuery}`);
        const [items, countResults] = await Promise.all([
            db
                .select({
                id: posts.id,
    ...
    async attachUserVotes(items, voterHash) {
        if (!items.length)
            return items;
        ...
        return items.map((item) => ({
            ...item,
            userVote: voteMap.get(item.id) || 0,
        }));
    }
}
