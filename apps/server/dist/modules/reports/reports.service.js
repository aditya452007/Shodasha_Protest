...
export class ReportsService {
    async submitReport(reporterHash, reason, postId, commentId, details) {
        return await db.transaction(async (tx) => {
            ...
            if (postId) {
                ...
                if (targetPost) {
                    ...
                    const newStatus = newReportCount >= REPORT_AUTO_HIDE_THRESHOLD ? 'quarantined' : targetPost.status;
                    await tx
                        .update(posts)
                        .set({
            ...
            if (commentId) {
                ...
                if (targetComment) {
                    ...
                    const newStatus = newReportCount >= REPORT_AUTO_HIDE_THRESHOLD ? 'hidden' : targetComment.status;
                    await tx
                        .update(comments)
                        .set({
                        reportCount: newReportCount,
